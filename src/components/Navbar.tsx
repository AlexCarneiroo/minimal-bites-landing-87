...
const Navbar = () => {
  ...
  return (
    <>
      <nav className="fixed top-0 left-0 right-0 z-50 bg-black/50 backdrop-blur-lg shadow-sm border-b border-white/20 transition-all duration-300">
          ...
          <div className="flex items-center space-x-4">
            ...
            >
              <div className="relative w-6 h-6">
                <Menu
                  style={{ color: primariColor }}
                  className={`h-6 w-6 absolute transition-all duration-300 ${isMobileMenuOpen ? 'rotate-180 opacity-0' : 'rotate-0 opacity-100'
                    }`}
                />
                <X
                  style={{ color: primariColor }}
                  className={`h-6 w-6 absolute transition-all duration-300 ${isMobileMenuOpen ? 'rotate-0 opacity-100' : 'rotate-180 opacity-0'
                    }`}
                />
              </div>
            </button>
        ...
        {/* Mobile menu */}
        <div className={`md:hidden bg-black/30 backdrop-blur-lg border-t border-white/20 transition-all duration-300 ease-in-out ${isMobileMenuOpen ? 'max-h-screen opacity-100' : 'max-h-0 opacity-0 overflow-hidden'
          }`}>
